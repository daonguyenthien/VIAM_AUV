...
void MissionProtocol::downloadMissionFromVehicle(const mavlink_message_t* msg)
{
  ...
  case 43: // MISSION_REQUEST_LIST
  {
    ...
    if (check_for_target_id_matching(reqPack, trans->autopilot_sysid, trans->autopilot_compid))
    {
      ...
      missionType = static_cast<uint8_t>(reqPack.mission_type);
      itemCount = static_cast<uint16_t>(itemList.waypoints.size());

      MISSION_COUNT resPack;
      resPack.count = itemCount;
      resPack.mission_type = missionType;
      pack_and_send_mavlink_message_t(resPack, link);
    }
  }
  ...
  case 51: // MISSION_REQUEST_INT
  {
    ...
    if (check_for_target_id_matching(reqPack, trans->autopilot_sysid, trans->autopilot_compid))
    {
      ...
      MISSION_ITEM_INT resPack;
      uint16_t currSeq = reqPack.seq;

      resPack.seq = currSeq;
      resPack.frame = itemList.waypoints[currSeq].frame;
      resPack.command = itemList.waypoints[currSeq].command;
      resPack.current = itemList.waypoints[currSeq].is_current;
      resPack.param1 = itemList.waypoints[currSeq].param1;
      resPack.param2 = itemList.waypoints[currSeq].param2;
      resPack.param3 = itemList.waypoints[currSeq].param3;
      resPack.param4 = itemList.waypoints[currSeq].param4;
      resPack.x = static_cast<int32_t>(itemList.waypoints[currSeq].x_lat);
      resPack.y = static_cast<int32_t>(itemList.waypoints[currSeq].y_long);
      resPack.z = static_cast<float>(itemList.waypoints[currSeq].z_alt);
      resPack.mission_type = missionType;

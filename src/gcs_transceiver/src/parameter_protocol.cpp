...
{
  speedPID = {"speed_Kp", "speed_Ki", "speed_Kd"};
  ...
  ros::NodeHandle nh;
  reqSetSpeedPID = nh.serviceClient<ParamSet>("parameter/set_speed_PID");
  reqGetSpeedPID = nh.serviceClient<ParamGet>("parameter/get_speed_PID");
...
void ParameterProtocol::readAllParameters(const mavlink_message_t* msg)
{
  ...
  if (check_for_target_id_matching(reqPack, trans->autopilot_sysid, trans->autopilot_compid))
  {
    ...
    uint16_t numParams = static_cast<uint16_t>(speedPID.size() + headingPID.size() + setpointParams.size() + LOSParams.size());
    uint16_t paramId = 0;

    readParamList(speedPID, numParams, paramId);
...
void ParameterProtocol::readParamList(const StringList& paramList, const uint16_t& numParams, uint16_t& paramId)
{
  ...
  for (auto it = paramList.begin(); it != paramList.end(); it++)
  {
    ...
    if (requestGetParameter(srv, paramList))
    {
      ...
      if (paramList == speedPID || paramList == headingPID || paramList == setpointParams || paramList == LOSParams)
      {
        resPack.param_value = static_cast<float>(srv.response.value.real);
...
{
  if (paramListMatched(speedPID, srv.request.param_id.data()))
    return reqSetSpeedPID.call(srv);
...
{
  if (paramList == speedPID)
    return reqGetSpeedPID.call(srv);

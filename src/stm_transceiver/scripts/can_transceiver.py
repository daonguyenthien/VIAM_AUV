...
def send_one(bus):
    ...
    headers = [[79, 76, 82 if (thruster_pwm < 0) else 76], [82, 85, 68], [79, 76, 77], [79, 76, 80], [67,79,78 if motorsLocked else 70 ], [69, 74, 83],[69, 74, 83]]
    values = [abs(thruster_pwm), abs(rudder_pwm), abs(mass_shifter_pwm), piston_position_pwm,0, 0,0]
    for i in range(13):
        if (i < 7):
            header = headers[i]
        ...
        if i >= 6:
            ...
            if frame is not None:
                ...
                if frame.arbitration_id == 290:
                    ...
                    #print(BoardARM2Status.rudder_position)
                    pub_rudder.publish(_BoardARM2Status)
                    #rint("RudderStatus " + str(data))
...
if __name__=="__main__":
    ...
    pub_thruster = rospy.Publisher('status/thruster', ThrusterStatus, queue_size=10)
    pub_rudder = rospy.Publisher('status/board_arm2', BoardARM2Status, queue_size=10)
    pub_mass = rospy.Publisher('status/mass_shifter', MassShifterStatus, queue_size=10)
    pub_piston = rospy.Publisher('status/piston', PistonStatus, queue_size=10)
    pub_altimeter = rospy.Publisher('status/board_arm1',BoardARM1Status,queue_size=10)
